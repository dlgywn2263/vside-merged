...
export default function MenuBar({ mode = "personal" }) {
  ...
  const { isTerminalVisible, breakpoints, codeMapMode } = useSelector(
    (state) => state.ui,
  );
  ...
  const handleCreateBranch = async () => {
    ...
    try {
      ...
      console.error("handleCreateBranch error:", error);
      alert(
  ...
  const handleDeleteBranch = async (e, branchName) => {
    ...
    try {
      ...
      console.error("handleDeleteBranch error:", error);
      alert(
        error instanceof Error
          ? error.message
          : typeof error === "string"
  ...
  const executeCreateSandbox = async () => {
    ...
    try {
      ...
      console.error("executeCreateSandbox error:", error);
      alert(
        error instanceof Error
          ? error.message
          : typeof error === "string"
  ...
  const executeApplySandbox = async () => {
    ...
    try {
      ...
      console.error("executeApplySandbox error:", error);
      alert(
        error instanceof Error
          ? `⚠️ 병합 실패:\n${error.message}`
          : typeof error === "string"
  ...
  const handleSendInvite = async () => {
    ...
    try {
      ...
      console.error("handleSendInvite error:", error);
      alert(
        error instanceof Error
          ? `초대 실패: ${error.message}`
          : typeof error === "string"
            ? `초대 실패: ${error}`
            : `초대 실패: ${JSON.stringify(error, null, 2)}`,
      );
    } finally {
      setIsInviting(false);
    }
  ...
  const handleMenuItemClick = async (menuName, itemName) => {
    ...
    switch (itemName) {
      ...
      case "디버깅 없이 실행":
        ...
        dispatch(
          writeToTerminal(
            `[System] ${language} 환경에서 ${activeFileId} 실행을 준비합니다...\r\n`,
          ),
        );
  ...
  return (
    <>
      <div>
        {!isRelocationPage && (
          <div
            className={`flex items-center justify-between px-4 h-9 border-t relative transition-colors duration-700 ${
              isSandboxMode
          ...
          >
            <div
              className={`flex items-center gap-1 text-[13px] ${
                isSandboxMode ? "text-indigo-200" : "text-gray-700"
              }`}
            >
              {subMenus.map((menu) => (
              ...
              {mode === "team" && (
                <div
                  className={`flex items-center gap-1.5 ml-3 border-l pl-3 animate-fade-in ${
                    isSandboxMode ? "border-indigo-700/50" : "border-gray-300"
                  }`}
                >
                  <div
                    onClick={() => setIsTeamModalOpen(true)}
                    className={`cursor-pointer px-3 py-1 rounded-md font-extrabold flex items-center gap-1.5 transition-all border active:scale-95 ${
                      isSandboxMode
                        ? "bg-indigo-500/20 text-indigo-300 border-indigo-500/30 hover:bg-indigo-500/40"
                        : "text-blue-600 bg-blue-50/80 hover:bg-blue-100 border-blue-100"
                    }`}
                  >
                    TEAM
                  </div>
                  ...
                  <div
                    ...
                    className={`cursor-pointer px-3 py-1 rounded-md font-extrabold flex items-center gap-1.5 transition-all border active:scale-95 ${
                      isSandboxMode
                        ...
                        : "text-green-600 bg-green-50/80 hover:bg-green-100 border-green-100"
                    }`}
                  >
            ...
            <div className="flex items-center gap-3">
              <button
                ...
                className={`flex items-center gap-1 text-[12px] font-bold px-2.5 py-1 rounded transition-colors ${
                  isSandboxMode
              ...
              <div
                className={`w-[1px] h-4 mx-1 ${
                  isSandboxMode ? "bg-indigo-800" : "bg-gray-300"
                }`}
              ></div>
              ...
              <div className="relative" ref={branchRef}>
                <div
                  className={`flex items-center gap-1.5 text-[11px] font-mono border px-3 py-0.5 rounded cursor-pointer transition-all duration-300 shadow-sm ${
                    isSandboxMode
                      ? "bg-indigo-900/50 border-indigo-500 text-indigo-300 hover:bg-indigo-800"
                      : isBranchOpen
                ...
                >
                  ...
                  <VscChevronDown
                    ...
                    } ${isSandboxMode ? "text-indigo-500" : "text-gray-400"}`}
                  />
                </div>
                      ...
                      {visibleBranches.map((branch) => {
                        ...
                        return (
                          <div
                            ...
                            className={`flex items-center justify-between px-3 py-2.5 cursor-pointer text-xs group transition-all rounded-lg ${
                              isActive
                                ? isFocus
                                  ? "bg-gradient-to-r from-indigo-50 to-purple-50 border border-indigo-100 text-indigo-700 font-bold shadow-sm"
                                  : "bg-gradient-to-r from-blue-50 to-cyan-50 border border-blue-100 text-blue-700 font-bold shadow-sm"
                                : isFocus
                                  ? "bg-indigo-50/30 text-indigo-600 hover:bg-indigo-50"
                                ...
                                <span className="relative flex h-2 w-2 ml-1">
                                  <span
                                    className={`animate-ping absolute inline-flex h-full w-full rounded-full opacity-75 ${
                                      isFocus ? "bg-indigo-400" : "bg-blue-400"
                                    }`}
                                  ></span>
                                  <span
                                    className={`relative inline-flex rounded-full h-2 w-2 ${
                                      isFocus ? "bg-indigo-500" : "bg-blue-500"
                                    }`}
                                  ></span>
                                </span>
                              )}
                  ...
                  <div className="flex -space-x-1.5">
                    {teamMembers.slice(0, 3).map((member, idx) => (
                      <div
                        ...
                        } border-2 border-white flex items-center justify-center text-[10px] text-white font-bold shadow-sm`}
                      >
                        {member.nickname?.[0]}
              ...
              <span
                className={`text-[11px] font-black px-3 py-0.5 rounded-full shadow-sm select-none tracking-wider border ${
                  isSandboxMode
              ...
              <div className="space-y-2">
                ...
                <div className="flex items-center gap-2 mt-2">
                  ...
                  <button
                    ...
                    className={`px-5 py-3 rounded-xl flex items-center gap-1.5 text-[12px] font-bold shrink-0 transition-all shadow-sm ${
                      isCopied
                        ? "bg-green-500 text-white border-transparent"
                        : "bg-white border border-gray-300 text-gray-800 hover:bg-gray-50 active:scale-95"
                    }`}
                  >
                    {isCopied ? (
                      <>
      ...
      {isVoiceChatModalOpen && mode === "team" && (
        <div
          className="fixed inset-0 bg-black/40 backdrop-blur-sm z-[9999] flex items-center justify-center animate-fade-in"
          onClick={() => setIsVoiceChatModalOpen(false)}
        >
          <div
            className="bg-white rounded-2xl shadow-2xl w-[360px] overflow-hidden flex flex-col animate-slide-up ring-1 ring-black/5"
            onClick={(e) => e.stopPropagation()}
          >
            <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100 bg-green-50/50">
              ...
              <button
                onClick={() => setIsVoiceChatModalOpen(false)}
                className="text-gray-400 hover:text-gray-800 bg-white hover:bg-gray-100 p-1.5 rounded-full transition-colors shadow-sm"
              >
                <VscClose size={20} />
              </button>
            </div>
