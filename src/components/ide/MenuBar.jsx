...
const VoiceChatRoom = ({ myUserId, teamMembers, onClose }) => {
  ...
  const myMember = teamMembers.find((m) => String(m.userId) === String(myUserId));
    ...
    <div className="flex flex-col h-[480px]">
      <div className="px-6 py-4 flex justify-between items-center border-b border-[#1E1F22]/50 bg-[#2B2D31]">
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs font-bold text-gray-200">{isVoiceConnected ? "음성 서버 연결됨" : "연결이 끊겨있습니다"}</span>
        </div>
      </div>
      <div className="flex-1 p-6 overflow-y-auto custom-scrollbar grid grid-cols-3 gap-4 content-start">
        <div className="flex flex-col items-center gap-2">
          <div className={`relative w-16 h-16 rounded-full flex items-center justify-center text-xl font-black text-white shadow-lg transition-all duration-200 ${isMutedLocal || !isVoiceConnected ? "bg-gray-600 opacity-60" : "bg-indigo-500"}`}>
            {myNickname[0]}
          ...
          <span className="text-[12px] font-bold text-gray-300 bg-[#1E1F22] px-2 py-0.5 rounded-md truncate max-w-full">{myNickname} (나)</span>
        </div>
        {isVoiceConnected && teamMembers.filter((m) => String(m.userId) !== String(myUserId)).map((member) => (
          <div key={member.userId} className="flex flex-col items-center gap-2 animate-fade-in">
            <div className="w-16 h-16 bg-gray-500 rounded-full flex items-center justify-center text-xl font-black text-white shadow-lg transition-all duration-200">{member.nickname[0]}</div>
            <span className="text-[12px] font-bold text-gray-300 bg-[#1E1F22] px-2 py-0.5 rounded-md truncate max-w-full">{member.nickname}</span>
          </div>
        ))}
      </div>
      <div className="bg-[#232428] px-6 py-5 flex justify-center items-center gap-6 rounded-b-2xl">
        <button onClick={() => setIsMutedLocal(!isMutedLocal)} className={`w-14 h-14 rounded-full flex items-center justify-center transition-all shadow-md active:scale-90 ${isMutedLocal ? "bg-red-500 text-white" : "bg-[#383A40] hover:bg-[#474A52] text-gray-200"}`} title={isMutedLocal ? "마이크 켜기" : "마이크 끄기"}>
          {isMutedLocal ? <VscMute size={24} /> : <VscMicFilled size={24} />}
        </button>
        <button onClick={handleConnectToggle} className={`w-14 h-14 rounded-full text-white flex items-center justify-center transition-all shadow-md active:scale-90 ${isVoiceConnected ? "bg-red-500 hover:bg-red-600" : "bg-emerald-500 hover:bg-emerald-600"}`} title={isVoiceConnected ? "연결 끊기" : "채널 접속하기"}>
...
export default function MenuBar({ mode = "personal" }) {
  ...
  const { workspaceId, activeProject, activeBranch, fileContents, activeFileId, tree } = useSelector((state) => state.fileSystem);
  
  // 💡 [NEW] isRunning, isDebugMode 상태 추출 추가
  const { isTerminalVisible, breakpoints, codeMapMode, isVoiceConnected, isRunning, isDebugMode } = useSelector((state) => state.ui);
  ...
  useEffect(() => {
    if (user && user.id) {
      getUserProfileApi(user.id).then(setMyProfile).catch((err) => console.error("프로필 정보 로드 실패", err));
    }
  }, [user]);
  ...
  useEffect(() => {
    const handleEsc = (e) => {
      if (e.key === "Escape") { setIsTeamModalOpen(false); setIsVoiceChatModalOpen(false); setIsInviteModalOpen(false); setIsSandboxCreateModalOpen(false); setIsSandboxApplyModalOpen(false); }
    };
  ...
  useEffect(() => {
    if (workspaceId && activeProject) fetchBranchListApi(workspaceId, activeProject).then(setBranches).catch(console.error);
    else setBranches([]);
  }, [workspaceId, activeProject, isBranchOpen, currentBranch]);

  useEffect(() => {
    if (mode === "team" && workspaceId) getWorkspaceMembersApi(workspaceId).then(setTeamMembers).catch(console.error);
  }, [mode, workspaceId, isTeamModalOpen]);

  const handleSelectBranch = (branchName) => {
    if (branchName === activeBranch) return;
    dispatch(closeAllFiles()); dispatch(clearVirtualTree()); dispatch(setActiveBranch(branchName)); setIsBranchOpen(false);
  };

  const handleCreateBranch = async () => {
    if (!newBranchName.trim()) return;
    try {
      setIsCreatingBranch(true); await createBranchApi(workspaceId, activeProject, newBranchName);
      dispatch(closeAllFiles()); dispatch(clearVirtualTree()); dispatch(setActiveBranch(newBranchName)); setNewBranchName(""); setIsBranchOpen(false);
    } catch (error) { alert(error.message); } finally { setIsCreatingBranch(false); }
  };
  ...
  const handleDeleteBranch = async (e, branchName) => {
    ...
    try {
      ...
      if (activeBranch === branchName) { dispatch(closeAllFiles()); dispatch(clearVirtualTree()); dispatch(setActiveBranch("master")); setIsBranchOpen(false); }
    } catch (error) { alert(error.message); }
  };

  const executeCreateSandbox = async () => {
    if (!sandboxTaskName.trim()) return alert("작업명을 입력해주세요.");
    setIsSandboxCreateModalOpen(false); setFullScreenLoading({ isOpen: true, text: "격리된 샌드박스 환경을 구축하는 중입니다..." });
    try {
      const sandboxBranchName = await createSandboxApi(workspaceId, activeProject, currentNickname, sandboxTaskName);
      dispatch(closeAllFiles()); dispatch(clearVirtualTree()); dispatch(setActiveBranch(sandboxBranchName)); setSandboxTaskName("");
    } catch (error) { alert(error.message); } finally { setTimeout(() => setFullScreenLoading({ isOpen: false, text: "" }), 500); }
  };

  const executeApplySandbox = async () => {
    if (!mergeCommitMessage.trim()) return alert("병합 전 남길 커밋 메시지를 입력해주세요!");
    setIsSandboxApplyModalOpen(false); setFullScreenLoading({ isOpen: true, text: "작업 내용을 저장하고 메인으로 합치는 중..." });
    try {
      if (fileContents && Object.keys(fileContents).length > 0) {
        const savePromises = Object.entries(fileContents).map(([path, content]) => saveFileApi(workspaceId, activeProject, activeBranch, path, content));
        await Promise.all(savePromises);
      }
      await applySandboxApi(workspaceId, activeProject, activeBranch, mergeCommitMessage, currentNickname);
      dispatch(closeAllFiles()); dispatch(setActiveBranch("master"));
      ...
      dispatch(clearVirtualTree()); setMergeCommitMessage("");
    } catch (error) { alert(`⚠️ 병합 실패:\n${error.message}`); } finally { setTimeout(() => setFullScreenLoading({ isOpen: false, text: "" }), 500); }
  };

  const handleCopyCode = () => {
    if (!workspaceId) return alert("워크스페이스 ID를 찾을 수 없습니다.");
    navigator.clipboard.writeText(workspaceId); setIsCopied(true); setTimeout(() => setIsCopied(false), 2000);
  };

  const handleSendInvite = async () => {
    if (!inviteEmail.trim()) return alert("초대할 이메일 주소를 입력해주세요.");
    try {
      setIsInviting(true); await inviteWorkspaceMemberApi(workspaceId, inviteEmail);
      alert(`✨ ${inviteEmail} 님에게 초대장을 발송했습니다!`); setInviteEmail(""); setIsInviteModalOpen(false);
    } catch (error) { alert(`초대 실패: ${error.message}`); } finally { setIsInviting(false); }
  };
  ...
  const handleQuickRun = async () => {
    ...
    if (tree && tree.children) {
        ...
        if (projectNode && projectNode.children) {
            ...
            if (rootFiles.includes("build.gradle")) {
                ...
                templateType = "REACT";
            } else if (rootFiles.includes("index.html") && !rootFiles.includes("package.json")) {
    ...
    const runPayload = { 
      type: "RUN", workspaceId, projectName: activeProject, branchName: activeBranch || "master", 
      filePath: activeFileId, language, templateType 
    };
  ...
  const startDebugSession = async () => {
    ...
    if (!isTerminalVisible) dispatch(toggleTerminal()); dispatch(setDebugMode(true)); dispatch(setActiveBottomTab("output"));
    try {
      const content = fileContents[activeFileId] || "";
      await saveFileApi(workspaceId, activeProject, activeBranch || "master", activeFileId, content);
      dispatch(writeToTerminal(`\r\n[System] 코드를 자동 저장했습니다: ${activeFileId}\r\n`));
    ...
    DebugSocket.connect(`${WS_BASE}/ws/debug`, () => { DebugSocket.startDebug(workspaceId, activeProject, activeBranch || "master", activeFileId, currentFileBreakpoints); }, (message) => { dispatch(writeToTerminal(message)); }, () => { dispatch(writeToTerminal("\r\n[System] 디버깅 세션이 종료되었습니다.\r\n")); dispatch(setDebugMode(false)); });
  };

  const handleMenuItemClick = async (menuName, itemName) => {
    setActiveMenu(null);
    switch (itemName) {
      case "새 파일": if (!isTerminalVisible) dispatch(toggleTerminal()); dispatch(writeToTerminal("[System] 새 파일을 생성합니다.\n")); break;
      case "파일 열기...": case "폴더 열기...": case "탐색기": dispatch(toggleSidebar()); break;
      case "저장":
        if (!activeFileId || !workspaceId || !activeProject) return alert("저장할 파일이나 프로젝트가 선택되지 않았습니다.");
        try {
          const content = fileContents[activeFileId] || ""; await saveFileApi(workspaceId, activeProject, activeBranch || "master", activeFileId, content);
          if (!isTerminalVisible) dispatch(toggleTerminal()); dispatch(writeToTerminal(`[System] Saved: ${activeFileId}\n`));
        } catch (error) { if (!isTerminalVisible) dispatch(toggleTerminal()); dispatch(writeToTerminal(`[Error] Save failed: ${error.message}\n`)); }
        break;
      
      case "다른 이름으로...": case "모두 저장": case "내보내기":
      case "검색": case "소스 제어": case "실행 및 디버그": case "확장":
      ...
      case "닫기": dispatch(closeFile(activeFileId)); break;
      ...
      case "출력": if (!isTerminalVisible) dispatch(toggleTerminal()); dispatch(setActiveBottomTab("output")); break;
      case "디버그 콘솔": case "터미널": case "새 터미널": case "터미널 분할": if (!isTerminalVisible) dispatch(toggleTerminal()); dispatch(setActiveBottomTab("terminal")); dispatch(writeToTerminal(`[System] ${itemName} 세션을 엽니다.\n$ `)); break;
      ...
      case "참조로 이동": if (!isTerminalVisible) dispatch(toggleTerminal()); dispatch(writeToTerminal("[System] 참조로 이동 기능 준비 중입니다.\n")); break;
      case "줄로 이동...": dispatch(triggerEditorCmd("go_to_line")); break;
      
      case "디버깅 시작": await startDebugSession(); break;
      
      // 💡 [NEW] 메뉴 클릭 시에도 빠른 실행/중지 함수를 재활용
      case "디버깅 없이 실행": await handleQuickRun(); break;
      case "디버깅 중지": handleQuickStop(); break;
      ...
      case "프로젝트 빌드": case "다시 빌드":
        if (!workspaceId || !activeProject) return alert("빌드할 프로젝트를 탐색기에서 선택해주세요!");
        if (!isTerminalVisible) dispatch(toggleTerminal()); dispatch(setActiveBottomTab("output")); dispatch(writeToTerminal(`\r\n[System] 🔨 ${activeProject} 프로젝트 빌드를 시작합니다...\r\n`));
        fetch(`${BASE_URL}/api/workspaces/build`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ workspaceId, projectName: activeProject, branchName: activeBranch || "master" }) })
          .then(async (res) => {
            ...
            if (disposition && disposition.includes("filename=")) {
              ...
              if (matches != null && matches[1]) { filename = matches[1].replace(/['"]/g, ""); try { filename = decodeURIComponent(filename); } catch {} }
            }
      ...
      case "빌드 취소": if (!isTerminalVisible) dispatch(toggleTerminal()); dispatch(writeToTerminal("[System] 빌드를 취소했습니다.\n")); break;
      case "전체 화면": dispatch(setCodeMapMode("full")); dispatch(openCodeMapTab()); break;
      case "분할 화면": dispatch(setCodeMapMode("split")); dispatch(openCodeMapTab()); break;
      default: break;
    }
  };
  ...
  const visibleBranches = branches.filter((branch) => {
    if (branch.startsWith("focus-") || branch.startsWith("focus/")) return branch.startsWith(`focus-${currentNickname}-`) || branch.startsWith(`focus/${currentNickname}/`);
    return true;
  });
  ...
  return (
    <>
        ...
        {!isRelocationPage && (
          <div className={`flex items-center justify-between px-4 h-9 border-t relative transition-colors duration-700 ${isSandboxMode ? "bg-slate-900 border-indigo-900/50" : "bg-[#f8f9fa] border-gray-100"}`} ref={menuRef}>
            <div className={`flex items-center gap-1 text-[13px] ${isSandboxMode ? "text-indigo-200" : "text-gray-700"}`}>
              {subMenus.map((menu) => (
                <div key={menu.name} className="relative">
                  <div className={`cursor-pointer px-3 py-1 rounded transition-colors ${activeMenu === menu.name ? isSandboxMode ? "bg-indigo-800 text-white font-medium" : "bg-gray-200 font-medium" : isSandboxMode ? "hover:bg-indigo-800/50" : "hover:bg-gray-200"}`} onClick={() => setActiveMenu(activeMenu === menu.name ? null : menu.name)}>{menu.name}</div>
                  {activeMenu === menu.name && (
                    <div className="absolute left-0 top-full mt-1 w-56 bg-white border border-gray-200 shadow-[0_8px_30px_rgba(0,0,0,0.12)] rounded-md py-1.5 z-[9999] animate-fade-in-up">
                      {menu.items.map((item, idx) => (
                        <div key={idx} onClick={() => handleMenuItemClick(menu.name, item.label)} className="px-5 py-1.5 hover:bg-blue-50 hover:text-blue-600 cursor-pointer text-[13px] text-gray-700 transition-colors flex justify-between items-center">
              ...
              {mode === "team" && (
                <div className={`flex items-center gap-1.5 ml-3 border-l pl-3 animate-fade-in ${isSandboxMode ? "border-indigo-700/50" : "border-gray-300"}`}>
                  <div onClick={() => setIsTeamModalOpen(true)} className={`cursor-pointer px-3 py-1 rounded-md font-extrabold flex items-center gap-1.5 transition-all border active:scale-95 ${isSandboxMode ? "bg-indigo-500/20 text-indigo-300 border-indigo-500/30 hover:bg-indigo-500/40" : "text-blue-600 bg-blue-50/80 hover:bg-blue-100 border-blue-100"}`}>TEAM</div>
                  <div onClick={() => setIsVoiceChatModalOpen(true)} className={`cursor-pointer px-3 py-1 rounded-md font-extrabold flex items-center gap-1.5 transition-all border active:scale-95 ${isSandboxMode ? "bg-emerald-500/20 text-emerald-300 border-emerald-500/30 hover:bg-emerald-500/40" : "text-green-600 bg-green-50/80 hover:bg-green-100 border-green-100"}`}>
                    <span className="relative flex h-2 w-2">
                      <span className={`animate-ping absolute inline-flex h-full w-full rounded-full opacity-75 ${isVoiceConnected ? "bg-emerald-400" : "bg-gray-500"}`}></span>
            ...
            <div className="flex items-center gap-3">
              ...
              <button onClick={() => dispatch(openProjectModal())} className={`flex items-center gap-1 text-[12px] font-bold px-2.5 py-1 rounded transition-colors ${isSandboxMode ? "bg-indigo-600 text-white hover:bg-indigo-500" : "bg-[#333] text-white hover:bg-black"}`}>
              ...
              <div className="flex items-center gap-1.5 mr-2">
                {mode === "team" && activeProject && currentBranch === "master" && (
                  <button onClick={() => setIsSandboxCreateModalOpen(true)} className="group flex items-center gap-1.5 px-3 py-1 bg-gradient-to-r from-purple-100 to-indigo-50 text-indigo-700 hover:from-purple-200 hover:to-indigo-100 border border-indigo-200 rounded text-[11px] font-extrabold transition-all shadow-sm active:scale-95">
                    <VscLock className="group-hover:scale-110 transition-transform" size={14} /> 샌드박스 (혼자 작업)
                  </button>
                )}
                {mode === "team" && activeProject && isSandboxMode && (
                  <button onClick={() => setIsSandboxApplyModalOpen(true)} className="group flex items-center gap-1.5 px-3 py-1 bg-gradient-to-r from-emerald-500 to-teal-400 text-white hover:from-emerald-400 hover:to-teal-300 border border-emerald-400/50 rounded text-[11px] font-extrabold transition-all shadow-[0_0_10px_rgba(16,185,129,0.4)] hover:shadow-[0_0_15px_rgba(16,185,129,0.6)] animate-pulse hover:animate-none active:scale-95">
              ...
              <div className="relative" ref={branchRef}>
                <div className={`flex items-center gap-1.5 text-[11px] font-mono border px-3 py-0.5 rounded cursor-pointer transition-all duration-300 shadow-sm ${isSandboxMode ? "bg-indigo-900/50 border-indigo-500 text-indigo-300 hover:bg-indigo-800" : isBranchOpen ? "bg-blue-50 border-blue-300 text-blue-700" : "bg-white border-gray-200 text-gray-600 hover:bg-gray-50"}`} onClick={() => { if (!activeProject) return alert("좌측 탐색기에서 시작 프로젝트를 먼저 설정해주세요!"); setIsBranchOpen(!isBranchOpen); }}>
                  ...
                  <div className="absolute right-0 top-full mt-2 w-72 bg-white border border-gray-200 shadow-[0_10px_40px_-10px_rgba(0,0,0,0.2)] rounded-xl py-2 z-[9999] animate-fade-in-up origin-top-right">
                      ...
                      {visibleBranches.map((branch) => {
                        ...
                        return (
                          <div key={branch} onClick={() => handleSelectBranch(branch)} className={`flex items-center justify-between px-3 py-2.5 cursor-pointer text-xs group transition-all rounded-lg ${isActive ? isFocus ? "bg-gradient-to-r from-indigo-50 to-purple-50 border border-indigo-100 text-indigo-700 font-bold shadow-sm" : "bg-gradient-to-r from-blue-50 to-cyan-50 border border-blue-100 text-blue-700 font-bold shadow-sm" : isFocus ? "bg-indigo-50/30 text-indigo-600 hover:bg-indigo-50" : "text-gray-600 hover:bg-gray-50 border border-transparent"}`}>
                            ...
                            {branch !== "master" && (
                              <button onClick={(e) => handleDeleteBranch(e, branch)} className="text-gray-400 hover:text-red-500 hover:bg-red-50 p-1.5 rounded transition-all opacity-0 group-hover:opacity-100" title="브랜치 삭제"><VscTrash size={14} /></button>
                            )}
                          </div>
                        );
                      })}
                    ...
                    <div className="px-4 pt-3 border-t border-gray-100 mt-2 bg-gray-50/50 rounded-b-xl pb-1">
                      ...
                      <div className="flex items-center gap-1.5">
                        <input type="text" placeholder="새 브랜치 이름" value={newBranchName} onChange={(e) => setNewBranchName(e.target.value)} className="flex-1 text-xs border border-gray-300 rounded-md px-2.5 py-1.5 outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-100 transition-all bg-white" />
                        <button onClick={handleCreateBranch} disabled={isCreatingBranch || !newBranchName.trim()} className="bg-gray-800 text-white p-1.5 rounded-md hover:bg-black transition-all disabled:opacity-50 active:scale-95">
                ...
                <div className="relative group flex items-center cursor-help ml-2 mr-1">
                  <div className="flex -space-x-1.5">
                    {teamMembers.slice(0, 3).map((member, idx) => (
                      <div key={member.userId} className={`w-7 h-7 rounded-full ${avatarColors[idx % avatarColors.length]} border-2 border-white flex items-center justify-center text-[10px] text-white font-bold shadow-sm relative`}>
                        {member.nickname?.[0]}<div className="absolute bottom-0 right-0 w-2 h-2 bg-green-500 border-2 border-white rounded-full"></div>
                      </div>
                    ))}
                    {teamMembers.length > 3 && (
                      <div className="w-7 h-7 rounded-full bg-gray-500 border-2 border-white flex items-center justify-center text-[10px] text-white font-bold shadow-sm">+{teamMembers.length - 3}</div>
                    )}
                  </div>
                    ...
                    <div className="space-y-2">
                      {teamMembers.map((member) => (
                        <div key={member.userId} className="flex items-center justify-between">
                          <div className="flex items-center gap-2">
                            <div className="w-2 h-2 bg-green-500 rounded-full"></div><span className="text-[12px] font-bold text-gray-800 truncate max-w-[80px]">{member.nickname}</span>
                          </div>
          ...
          <div className="bg-white rounded-2xl shadow-[0_20px_50px_rgba(0,0,0,0.3)] w-[440px] overflow-hidden flex flex-col animate-slide-up ring-1 ring-black/5" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="p-6 bg-white space-y-5">
              <div className="space-y-2">
                ...
                <input type="text" value={sandboxTaskName} onChange={(e) => setSandboxTaskName(e.target.value)} onKeyDown={(e) => e.key === "Enter" && executeCreateSandbox()} placeholder="예) 로그인 에러 수정, 헤더 UI 변경" className="w-full px-4 py-3 border border-gray-300 rounded-xl text-[14px] outline-none focus:border-indigo-500 focus:ring-4 focus:ring-indigo-100 transition-all font-medium" autoFocus />
              </div>
              <button onClick={executeCreateSandbox} disabled={!sandboxTaskName.trim()} className="w-full py-3.5 bg-indigo-600 hover:bg-indigo-700 active:scale-[0.98] text-white rounded-xl text-[14px] font-bold shadow-lg shadow-indigo-200 transition-all disabled:opacity-50 disabled:active:scale-100 flex items-center justify-center gap-2">공간 생성 및 이동하기</button>
            </div>
          </div>
        </div>
      )}
          ...
          <div className="bg-white rounded-3xl shadow-[0_20px_50px_rgba(0,0,0,0.4)] w-[460px] overflow-hidden flex flex-col animate-slide-up" onClick={(e) => e.stopPropagation()}>
            <div className="p-8 pb-6 text-center flex flex-col items-center border-b border-gray-100">
              <div className="w-16 h-16 bg-emerald-100 rounded-full flex items-center justify-center mb-5 border-4 border-white shadow-md relative">
                <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-emerald-200 opacity-50"></span><VscRocket className="text-emerald-600" size={32} />
              </div>
            ...
            <div className="p-6 bg-gray-50 space-y-3">
              ...
              <input type="text" value={mergeCommitMessage} onChange={(e) => setMergeCommitMessage(e.target.value)} onKeyDown={(e) => e.key === "Enter" && executeApplySandbox()} placeholder="예) 로그인 화면 레이아웃 수정 완료" className="w-full px-4 py-3 border border-gray-300 rounded-xl text-[13px] outline-none focus:border-emerald-500 focus:ring-2 focus:ring-emerald-100 transition-all font-medium bg-white" autoFocus />
            </div>
            <div className="flex border-t border-gray-100 p-4 gap-3 bg-white">
              <button onClick={() => setIsSandboxApplyModalOpen(false)} className="flex-1 py-3 bg-white border border-gray-200 text-gray-600 hover:bg-gray-50 active:scale-95 rounded-xl text-[13px] font-bold transition-all shadow-sm">취소</button>
              <button onClick={executeApplySandbox} disabled={!mergeCommitMessage.trim()} className="flex-[2] py-3 bg-emerald-500 hover:bg-emerald-600 active:scale-[0.98] text-white rounded-xl text-[13px] font-bold shadow-md shadow-emerald-200 transition-all flex items-center justify-center gap-2 disabled:opacity-50 disabled:active:scale-100"><VscCheck size={16} strokeWidth={1} /> 커밋 및 병합하기</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm z-[9998] flex items-center justify-center animate-fade-in" onClick={() => setIsTeamModalOpen(false)}>
          <div className="bg-white rounded-2xl shadow-2xl w-[400px] overflow-hidden flex flex-col animate-slide-up ring-1 ring-black/5" onClick={(e) => e.stopPropagation()}>
                    ...
                    <div className="flex items-center gap-3.5">
                      ...
                      <div className="flex flex-col">
                        <div className="flex items-center gap-2">
                          ...
                          <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded-sm ${member.role === "OWNER" ? "bg-blue-100 text-blue-700" : member.role === "ADMIN" ? "bg-orange-100 text-orange-700" : "bg-gray-100 text-gray-600"}`}>{member.role === "OWNER" ? "Owner" : member.role === "ADMIN" ? "Admin" : "Member"} {isMe && "(나)"}</span>
                        </div>
            ...
            <div className="p-4 border-t border-gray-100 bg-white">
              <button onClick={() => { setIsInviteModalOpen(true); setIsTeamModalOpen(false); }} className="w-full py-3 bg-blue-600 hover:bg-blue-700 active:scale-[0.98] text-white rounded-xl text-[13px] font-bold flex items-center justify-center gap-2 shadow-md shadow-blue-200 transition-all"><VscAdd size={16} strokeWidth={1} /> 새로운 팀원 초대하기</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-[9999] flex items-center justify-center animate-fade-in" onClick={() => setIsInviteModalOpen(false)}>
          <div className="bg-white rounded-2xl shadow-2xl w-[420px] overflow-hidden flex flex-col animate-slide-up ring-1 ring-black/5" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between px-6 py-5 border-b border-gray-100 bg-white">
              <div><h2 className="text-lg font-black text-gray-900">팀원 초대</h2><p className="text-xs text-gray-500 mt-1">이메일 발송 또는 프로젝트 코드로 초대하세요</p></div>
              ...
              <div className="space-y-4">
                <div className="space-y-1.5">
                  ...
                  <div className="flex gap-2">
                    ...
                    <button onClick={handleSendInvite} disabled={isInviting || !inviteEmail.trim()} className="px-5 bg-[#2d333b] hover:bg-black text-white rounded-xl text-[13px] font-bold transition-all shadow-sm disabled:opacity-50 flex items-center justify-center shrink-0">{isInviting ? <VscRefresh className="animate-spin" size={16} /> : "발송"}</button>
                  </div>
                </div>
              </div>
              <div className="flex items-center gap-4"><div className="h-px bg-gray-200 flex-1"></div><span className="text-[11px] font-bold text-gray-400">또는</span><div className="h-px bg-gray-200 flex-1"></div></div>
              <div className="space-y-2">
      ...
      {isVoiceChatModalOpen && mode === "team" && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-[9999] flex items-center justify-center animate-fade-in" onClick={() => setIsVoiceChatModalOpen(false)}>
          <div className="bg-[#2B2D31] rounded-2xl shadow-[0_0_40px_rgba(0,0,0,0.5)] w-[600px] overflow-hidden flex flex-col animate-slide-up" onClick={(e) => e.stopPropagation()}>
            <VoiceChatRoom workspaceId={workspaceId} myUserId={user?.id} teamMembers={teamMembers} onClose={() => setIsVoiceChatModalOpen(false)} />
          </div>
        </div>
      )}
    </>
